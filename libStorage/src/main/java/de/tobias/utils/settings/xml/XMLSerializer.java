...
public class XMLSerializer {
				...
				if (field.isAnnotationPresent(Key.class)) {
					...
					if (data != null) {
						...
						if (field.getType().isPrimitive() || field.getType() == String.class || field.getType().isEnum()) {
							...
							resElement = serializeArray(rootElement, data);
						} else {
	...
	public static Element serializeClass(Element rootElement, Object data, boolean nestedType) throws IllegalAccessException {
		for (Class<?> handlerClass : handlers.keySet()) {
			if (handlerClass.isAssignableFrom(data.getClass())) {
				...
				Element element = classHandler.serialize(rootElement, data);
				if (element != null) {
					if (nestedType)
	...
	private static Element serializeArray(Element rootElement, Object data) {
		...
		element.addAttribute(XMLFormatStrings.LENGTH_ATTR, String.valueOf(length));
