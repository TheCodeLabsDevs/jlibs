 ...
 * {@linkplain #setTickLabelFormatter(javafx.util.StringConverter) set an formatter}.
 * <p/>
 ...
 * <pre>
 * {
 * 	&#064;code ObservableList&lt;XYChart.Series&lt;Date, Number&gt;&gt; series = FXCollections.observableArrayList();
 * 
...
public final class DateAxis extends Axis<Date> {
	...
	/**
	 * Stores the min and max date of the list of dates which is used. If {@link #autoRanging} is true, these values are used as lower and
	 * upper bounds.
	 */
	...
	protected void setRange(Object range, boolean animating) {
		...
		if (animating) {
			...
			currentAnimationID = animator.animate(new KeyFrame(Duration.ZERO, new KeyValue(currentLowerBound, oldLowerBound.getTime()),
					new KeyValue(currentUpperBound, oldUpperBound.getTime())), new KeyFrame(Duration.millis(700), new KeyValue(
					currentLowerBound, lower.getTime()), new KeyValue(currentUpperBound, upper.getTime())));
	...
	protected List<Date> calculateTickValues(double v, Object range) {
		...
		// If there are at least three dates, check if the gap between the lower date and the second date is at least half the gap of the
		// second and third date.
		// Do the same for the upper bound.
		// If gaps between dates are to small, remove one of them.
		// This can occur, e.g. if the lower bound is 25.12.2013 and years are shown. Then the next year shown would be 2014 (01.01.2014)
		// which would be too narrow to 25.12.2013.
	...
	private List<Date> makeDatesEven(List<Date> dates, Calendar calendar) {
		...
		if (dates.size() > 2) {
			...
			for (int i = 0; i < dates.size(); i++) {
				...
				switch (actualInterval.interval) {
				case Calendar.YEAR:
					// If its not the first or last date (lower and upper bound), make the year begin with first month and let the months
	...
	/**
	 * The intervals, which are used for the tick labels. Beginning with the largest interval, the axis tries to calculate the tick values
	 * for this interval. If a smaller interval is better suited for, that one is taken.
	 */
