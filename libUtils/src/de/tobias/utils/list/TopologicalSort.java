...
public class TopologicalSort {
	...
	private static <T> void explore(T node, DirectedGraph<T> graph, List<T> result, Set<T> visited, Set<T> expanded) {
		if (visited.contains(node)) {
			...
			// if it hasn't been expanded, it means that we've just found a node
			// that is currently being explored,
			// and therefore is part of a cycle. In that case, we should report
			// an error.
			throw new IllegalArgumentException("A cycle was detected within the Graph when exploring node "
					+ node.toString());
		}
