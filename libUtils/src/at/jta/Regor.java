 ...
 *
 * <p>
 * Title: Class is 100% pur Java Registry handle
 * </p>
 *
 * <p>
 * Description: You can read, delete or create any key in the registry (when you have access rights). But you just can read/write and set
 * string values! The java.dll doesnt provide any other feature.<br>
 * <b>ATTENTION: Since version 3 you can also read/write dword, binary, expand and multi data - but this is implemented by calling the
 * regedit executeable with parameters and temporary files - so it might not success or have a deadlock!!!</b>
 * </p>
 *
 * <p>
 * Copyright: Copyright (c) 2009 - class is under GPL and LGPL
 * </p>
 *
 * <p>
 ...
 * @version 2.0 22.03.2007 Methods are renamed and now called by the function the are implementing and the document is now in english,
 *          instead of german<br>
 *          <br>
 * @version 3.0 03.06.2008 Replaced all int Key values with Key class for storing the path and added new methods for reading/writing dword,
 *          binary, multi and expand values (these new methods are tested under XP SP2 and Vista Ultimate x64 with admin privliges and UCL
 *          turned off, so if there are any bugs with other windows version please submit it to me - or just to say thank you or to donate
 *          ;-))<br>
 ...
 * @version 3.3 20.10.2008 found major bug in the method extractAnyValue - the method returned and value found with the name, not only for
 *          the right key<br>
 ...
 * @version 4.0 RC2 14.04.2009 From a discussion at the java-forum.org board, a member told me, that there is also a native command called
 *          "reg.exe" and vista doesnt need admin privileges when you run it! So the version checks if reg.exe is here! If not take
 *          regedit.exe reg.exe is faster, not so much memory consuming (while parsing cached keys) and under vista UAC is not such a big
 *          problem as with regedit.exe! The best result is, that you dont need to convert the data anymore (if you stil do it, because
 *          upgrading from older version, it doesnt matter - so the result will be the same)
 ...
 * @version 4.2 Release 23.02.2010 As i changed my workplace and i forgot to release the new source as jar-file, I finaly release a newer
 *          version, which should be Win7/Vista safe, because it uses reg.exe instead of regedit.exe (for non DWORD entries) I also switched
 *          from my JBuilder2k5 to Eclipse, because my new company didnt want to buy me a Jbuilder :( - at least SVN support is now better
 *          :D
 * @version 4.3 Release 30.04.2010 I checked in a debug version, so that it will not work under vista or win7 without admin privs, because
 *          it was not looking after reg.exe. I also found some parsing errors and fixed them..
 * @version 4.4 Release 17.05.2010 After a user told me, he has problems reading out binary entries, i discovered several bugs with vista
 *          and win7. It seems to be that the output from reg.exe has been changed. I just tested with XP and never with vista/win7, but now
 *          i made tests with win7 and everything seems to be ok - now ;)
 * @version 4.5 Release 03.04.2012 Issue mentioned by fischl-thomas at 26. January 2012 - on XP, REG_SZ was not working anymore (because of
 *          the previous update to Win7)
 *******************************************************************************************************************************/
...
final public class Regor {
	 ...
	 *             throws an registryerrorException when its not able to get a handle to the registry methods
	 * @throws NotSupportedOSException
	 ...
	 * @param key
	 *            Key The parent key handle obtained by openKey
	 * @param valueName
	 *            String the binary value name in the registry
	 * @param hexCommaData
	 *            String the string converted in hexadecimal signs separated with commas. Use
	 *            <code>String convertStringToHexComma(String plainString, false)</code> to get the hex comma separated data
	 * @throws RegistryErrorException
	 **********************************************************************************************************************************/
	 ...
	 *            String the binary value name in the registry
	 * @param plainData
	 *            String like you would see in the registry (without any spaces, etc..)
	 * @throws RegistryErrorException
	 *********************************************************************************************************************************/
	...
	/**********************************************************************************************************************************
	 * Method reads from the registry a BINARY value - this is made via Runtime.getRuntime().exec(regedit) and is not one of the best
	 * methods, but at least it doesnt need a dll
	 * 
	 * @since version 3 (03.06.2008 - guess who has birthday ;))
	 * @see <code>extractAnyValue</code> - method could have a deadlock
	 * @param key
	 *            Key the obtained key from the registry
	 * @param valueName
	 *            String the valueName of the binary entry which you want to read
	...
	public String readBinary(Key key, String valueName) throws RegistryErrorException {
		...
		if (valueName == null)
			throw new NullPointerException(
					"Valuename cannot be null, because the default value is always a STRING! If you want to read a String use readValue");
		if (nativeHandler == null)
			throw new RegistryErrorException("NativeHandler is not initalized!");
	 ...
	 *            Key the parent key handle obtained by openKey
	 * @param valueName
	 *            String the valuename of the dword entry
	 * @param hexData
	 *            String a hexadecimal String withouth comma or spaces (use <code>Long.toHexString(long)</code> to get a hex string)
	 * @throws RegistryErrorException
	 *********************************************************************************************************************************/
	 ...
	 *            Key the parent key handle obtained by openKey
	 * @param valueName
	 *            String the multi value name
	 * @param hexCommaZeroData
	 *            String the data to write converted in hex string separated by a comma with trailing zeros Use
	 *            <code>String convertStringToHexComma(String plainString, true)</code> to get the hex comma separated data with 0 signs
	 *            between
	 * @throws RegistryErrorException
	 **********************************************************************************************************************************/
	 ...
	 * @param valueName
	 *            String the multi value name
	 * @return String the HEXADECIMAL values separated by comma (use <code>String parseHexString(String)</code> to convert it the line
	...
	/******************************************************************************************************************************
	 * Reading every valueName (not only the string value) out of the registry handle (for maximum value index and maxValueNameLength use
	 * the getChildInformation method
	 * 
	 * @param key
	 *            the handle to the parent key obtained from openKey
	 * @param valueNameIndex
	 *            the index of the valueName name - starting from 0 going to the maximum count from the getChildInformation stored in array
	 *            index 2
	 * @param maxValueNameLength
	 *            maximum length of valueName name (used because for memory allocating in the java.dll - if you obtain the size from
	 *            getChildInformation increase the [4] int array by 1)
	...
	public byte[] _enumValueName(int key, int valueNameIndex, int maxValueNameLength) throws RegistryErrorException {
		try {
			return (byte[]) enumValue.invoke(null,
					new Object[] { new Integer(key), new Integer(valueNameIndex), new Integer(maxValueNameLength) });
	 ...
	 * 
	 * @param key
	 *            either one of the root nodes or a key obtained from openKey
	 * @param subkey
	 *            a string to a subkey - if the subkey is empty or null the information will be obtained from the given key
	...
	/******************************************************************************************************************************
	 * Reading the subkey name out of the registry (to obtain the count and the maxKeyNameLength use <code>getChildInformation</code> method
	 * 
	 * @param key
	 *            the handle to the key obtained by openKey
	 * @param subkeyIndex
	 *            index from the subkey from which you want to obtain the name (start with 0 - the maximum count you get from
	 *            getChildInformation method in array [0])
	 * @param maxKeyNameLength
	 *            the maximum length of a subkey name (used because for memory allocating in the java.dll - if you obtain the size from
	 *            getChildInformation increase the [3] int array by 1 )
	...
	/******************************************************************************************************************************
	 * Reads information about the current opened key (use it when you want to enumKey or enumValueName to determine the maximum key length
	 * and the count of keys)
	 * 
	 * @param key
	 *            the key which you obtained from openKey
	 * @return int[0] the count of the subkeys,[2] count of valuenames, [3] the maximum length of a subkey! the maximum length of valuename
	 *         is stored in[4] (for other operations you should increase the [3] or [4] value by 1 because of the terminating \0 in C -
	 *         because you handle with the java.dll) if nothing found or illegal key, the values are -1 of the array (at index 1 the value
	 *         would be 6 the other -1)
	 * @throws RegistryErrorException
	 *****************************************************************************************************************************/
	 ...
	 * @param key
	 *            the key obtained by openKey
	 * @param valueName
	 *            name of String value you want to delete (if the string is empty or null the default entry will be cleared)
	 * @return int
	 * @throws RegistryErrorException
	 *****************************************************************************************************************************/
	 ...
	 *            the key obtained by openKey
	 * @param valueName
	 *            name of String value you want to delete (if the string is empty or null the default entry will be deleted)
	 * @return int
	 * @throws RegistryErrorException
	 *****************************************************************************************************************************/
	...
	/******************************************************************************************************************************
	 * Method saves the specified string value (DO NOT USE THIS METHOD FOR READING BINARY, DWORD, MULTI OR EXPAND ENTRIES - JUST FOR SZ -
	 * STRING ENTRIES!!!) Method saves or create a simple character sequence (REG_SZ) If you want to change the default Value the valueName
	 * has to be null or nothing
	 * 
	 * @since version 4: 14.04.2009
	 * @param key
	 *            obtained by openKey
	 * @param valueName
	 *            the string value name in the registry you want to set
	 * @param value
	...
	/******************************************************************************************************************************
	 * Method set the specified string value (DO NOT USE THIS METHOD FOR READING BINARY, DWORD, MULTI OR EXPAND ENTRIES - JUST FOR SZ -
	 * STRING ENTRIES!!!) Methode setzt (oder erstellt) einen Wert auf eine Zeichenfolge Will man den defaulteintrag �ndern, so muss man
	 * valueName "" �bergeben
	 * 
	...
	/******************************************************************************************************************************
	 * Reads the value of a string value (DO NOT USE THIS METHOD FOR READING BINARY, DWORD, MULTI OR EXPAND ENTRIES - JUST FOR SZ - STRING
	 * ENTRIES!!!)
	 * 
	 * @param key
	 *            obtained from openKey
	 * @param valueName
	 *            the string value which you want to read (if you want to obtain the default entry the valueName should be empty or NULL)
	 * @return byte[] if found the data in the string value will be returned (to get a string use the class method parseValue(byte[])) on
	 ...
	 *            Key the key handle
	 * @param valueName
	 *            String when valueName is empty or null plain key will be returned, because only plain keys can have empty names
	 ...
	 * 
	 * @param key
	 *            the parent key obtained by openKey
	 * @param subkey
	 *            the key name you want to delete
	...
	/******************************************************************************************************************************
	 * Create new key/subkey in the registry with the specified name Attentition: if the key is successfully returned, you should close and
	 * open the key again, because the obtained key doesnt have a high access level (so maybe creating or deleting a key/value wouldn�t be
	 * successful)
	 * 
	 * @param key
	 *            handle to parent key obtained from openKey
	 * @param subkey
	 *            name of the key/subkey you want to create
	 ...
	 * 
	 * @param key
	 *            one of the registry root nodes - either HKEY_CLASSES_ROOT, HKEY_CURRENT_USER or HKEY_LOCAL_MACHINE
	 * @param subkey
	 *            the name of the key/subkey like SOFTWARE or HARDWARE - for subkeys use the \\ as delimiter f.e. : SOFTWARE\\MICROSOFT if
	 *            subkey name is "" or null it returns the handle to the root node
	 * @param security_mask
	 *            the security mask to handle with the opened key (see security mask doc at the begin for detailed information)
	 ...
	 *            the name of the key/subkey like SOFTWARE or HARDWARE - for subkeys use the \\ as delimiter f.e. : SOFTWARE\\MICROSOFT if
	 *            subkey name is "" or null it returns the handle to the root node
	 * @return Key null if not found or not allowed (attention here this methods allways uses the KEY_ALL_ACCESS security mask) on success
	 *         the handle to key will be returned
	 * @throws RegistryErrorException
	 *****************************************************************************************************************************/
	 ...
	 *            the name of the key/subkey like SOFTWARE or HARDWARE - for subkeys use the \\ as delimiter f.e. : SOFTWARE\\MICROSOFT if
	 *            subkey name is "" or null it returns the handle to the root node
	 * @return int -1 if not found or not allowed (attention here this methods allways uses the KEY_ALL_ACCESS security mask) on success the
	 *         handle to key will be returned
	 * @throws RegistryErrorException
	...
	/**********************************************************************************************************************************
	 * Method converts a hex given String (separated by comma) into a string For Multi entries, every entry ends with HEX 0 so you can split
	 * the lines
	 * 
	 * @param hexCommaString
	 *            String
	 * @param deleteNullSigns
	 *            boolean if you want to remove every 0 sign (delete null signs is needed for multi and expand entries, but not for binary
	 * @return String
	 *********************************************************************************************************************************/
	 ...
	 * @param plain
	 *            String
	 * @param appendNullSigns
	 *            boolean if you want to add null signs (needed for multi and expand entries, but not for binary entry)
	...
	/******************************************************************************************************************************
	 * intern method which obtain the methods via reflection from the java.util.prefs.WindowPreferences (tested with java 1.4, 1.5 and java
	 * 1.6)
	 * 
	 * @throws RegistryErrorException
	 *             exception is thrown if any method is not found or if the class is not found
	 *****************************************************************************************************************************/
	 ...
	 * 
	 * @throws RegistryErrorException
	 *             throws this exception when no reg.exe or regedit.exe is found
	 * @todo test vista UAC
	 *********************************************************************************************************************************/
	 ...
	 * 
	 * @param key
	 *            Key the registry key which should be cached + subchildren
	 * @throws RegistryErrorException
	 *********************************************************************************************************************************/
	 ...
	 * @param key
	 *            Key the registry key which should be cached + subchildren
	 * @param maximumChildren
	 *            int amount of the subchildren which should be cached (attention, this may create a java.lang.OutofMemory Error if you have
	 *            not enoguth memory use -Xmx512M or sth)
	 * @throws RegistryErrorException
	 *********************************************************************************************************************************/
	 ...
	 *             throws this exception if there is no such entry - returns null if it is not cached or caching is disabled
	 **********************************************************************************************************************************/
	...
	public static void main(String[] args) throws Exception {
		...
		if (false) // testing the new methods
		{
			...
			// regor.saveAnyValue("HKEY_LOCAL_MACHINE\\Software\\AVS3", "MULTI", MULTI_KEY,convertStringToHexComma("Das ist ein langer Test,
			// mal schauen was da nachher rauskommt \n dumdidum!", true));
		...
		// AT FIRST THE OLD WAY IS USED!!
		{
			...
			{
				byte buf[] = regor._readValue(key2, (String) l.get(x)); // get the information - if is not a string value, null will be
																		// returned
		...
		// HERE THE NEW METHOD IS USED
		{
			...
			for (int x = 0; l != null && x != l.size(); x++) // getting the String value from the valueNames
			{
				byte buf[] = regor.readValue(key2, (String) l.get(x)); // get the information - if is not a string value, null will be
																		// returned
				System.out.println(x + ": " + l.get(x) + " == " + Regor.parseValue(buf)); // parses the byte buffer to String
			}
			// example to access the default valueName - either null or ""
	...
	private final class RegeditHandler implements INativeRegistryHandler {
		...
		/********************************************************************************************************************************
		 * Method saves any variable to the registry via the regedit.exe and runtime - because its java 1.4 compatbile (otherwhise it would
		 * be the processbuilder)
		 * 
		 * @param path
		 *            String the registy path (without [])
		 * @param valueName
		 *            String the valuename to set
		 * @param type
		 *            String the type (BINARY, DWORD, MULTI, EXPAND)
		 * @param data
		 *            String the data which should be stored in the registry (it must be converted into the right format for the given type)
		...
		/**********************************************************************************************************************************
		 * Method extracts any variable from the registry via the regedit.exe and runtime - because its java 1.4 compatbile (otherwhise it
		 * would be the processbuilder)
		 * 
		 * @param path
		 *            String the registry path to the parent key
		 * @param valueName
		 *            String the valuename which should be read from the registry key
		 * @param appendType
		 *            boolean if the method should append the type - not needed with regedit.exe method, because the type is always added
		...
		public String extractAnyValue(String path, String valueName, boolean appendType) throws RegistryErrorException {
			...
			try {
				...
				// ATTENTION!! THESE COULD BE A DEADLOCK BECAUSE I WAITFOR THE END OF PROCESS HERE
				Runtime.getRuntime().exec("regedit /e " + f.getAbsolutePath() + " \"" + path + "\"").waitFor(); // <-- WAITING FOR END OF
																												// PROCESS
		 ...
		 * 
		 * @param key
		 *            String the registry key which should be cached + subchildren
		 * @param maximumChildren
		 *            int amount of the subchildren which should be cached (attention, this may create a java.lang.OutofMemory Error if you
		 *            have not enoguth memory use -Xmx512M or sth)
		 * @throws RegistryErrorException
		 *********************************************************************************************************************************/
		public void cacheKeys(String key, int maximumChildren) throws RegistryErrorException {
			...
			try {
				...
				// ATTENTION!! THESE COULD BE A DEADLOCK BECAUSE I WAITFOR THE END OF PROCESS HERE
				Runtime.getRuntime().exec("regedit /e " + f.getAbsolutePath() + " \"" + key + "\"").waitFor(); // <-- WAITING FOR END OF
																												// PROCESS
				_waitForFile(f); // wait until the file size is not increasing anymore
	...
	private final class RegHandler implements INativeRegistryHandler {
		 ...
		 *            String the valuename which should be read from the registry key
		 * @param appendType
		 *            boolean add the type to the return string
		 * @return String null if the valuename is not found or the path could not be exported - otherwhise the data from the registry
		 * @throws RegistryErrorException
		 *********************************************************************************************************************************/
		public String extractAnyValue(String path, String valueName, boolean appendType) throws RegistryErrorException {
			...
			try {
				f = File.createTempFile("regorexp", ".jta"); // creates tmp File for storing the registry key
				// ATTENTION!! THESE COULD BE A DEADLOCK BECAUSE I WAITFOR THE END OF PROCESS HERE
				Runtime.getRuntime().exec("cmd /c \"reg query \"" + path + "\" /v \"" + valueName + "\" > " + f.getAbsolutePath() + " 2>&1\"")
						.waitFor(); // <-- WAITING FOR END OF PROCESS
				_waitForFile(f); // wait until the file size is not increasing anymore
				br = new BufferedReader(new InputStreamReader(new FileInputStream(f)));
				String line = "";
				...
				while ((line = br.readLine()) != null) {
					if (line.equals(path)) {
						...
						if (items[0].equals(valueName)) {
							...
							// [1] = entry
							/*
							 * if (items[0].equals("REG_MULTI_SZ")) strRet.append(MULTI_KEY); //add this for older version else if
							 * (items[0].equals("REG_EXPAND_SZ")) strRet.append(EXPAND_KEY); else if (items[0].equals("REG_DWORD"))
							 * strRet.append(DWORD_KEY); else if (items[0].equals("REG_BINARY")) strRet.append(BINARY_KEY);
							 */
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					if (line.trim().length() > 0) // if found and not empty line
					{
						...
						else if (currentEntry != null && !line.startsWith("Error:  ")) // else its a value and if it doesnet starts with
																						// Error:
						{
							StringBuffer strRet = new StringBuffer();
							...
							// [1] = entry
							/*
							 * if(items[0].equals("REG_MULTI_SZ")) strRet.append(MULTI_KEY); //add this for older version else
							 * if(items[0].equals("REG_EXPAND_SZ")) strRet.append(EXPAND_KEY); else if(items[0].equals("REG_DWORD"))
							 * strRet.append(DWORD_KEY); else if(items[0].equals("REG_BINARY")) strRet.append(BINARY_KEY);
							 */
							strRet.append(items[1]);
